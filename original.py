...
def on_message(message):
    ...
    if message.text == 'Начать тест заново':
        ...
        if int(message.text) - 1 == int(questions_for_python[gen_iter]['right_answer']):
            right_answ += 1
            bot.send_message(message.from_user.id, "Правильно, переходим к следующему вопросу...")
        else:
            bot.send_message(message.from_user.id, f"Неправильно, правильным ответом было {questions_for_python[gen_iter]['right_answer'] + 1}, переходим к следующему вопросу...")
