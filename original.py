...
questions_for_python = [
    ...
    {'text': 'Для чего в Python используется встроенная функция enumerate()?', 'all_answers': ['Для определения количества элементов последовательности.', 'Для одновременного итерирования по самим элементам и их индексам.', 'Для сортировки элементов по значениям id.', 'Для определения качества элементов.'], 'right_answer': 1},
...
def start_chat(message):
    ...
    str = ""
    if is_started:
        str = "Вы уже проходите тест, если хотите начать заново введите 'Начать тест заново'"
    else:
        str = "Выберите тест для прохождения: \n"
